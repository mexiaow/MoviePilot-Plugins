...
| 兜底 Topic ID | 没匹配到规则时发到哪个话题 |
| 接管哪些通知类型 | 不选就是全部，选了就只处理选中的 |
| 分流规则 | 关键词命中后发到哪个话题 |
...
- 已经指定发到微信、Telegram、Web 的通知，插件不再转发。
- 没指定渠道的通知，插件才拿来发到 TG 话题。

关闭时：

- 插件会尽量接管所有符合类型的通知。
- 原本发微信的通知，也可能被额外发一份到 TG 话题。

默认建议开启。

如果你发现某些通知没有进入 Telegram 话题，可以关掉再试。
...
例如只想分流下载、入库、订阅，就只选这三个。

不确定怎么选，就先清空不选。

让所有通知进插件，再靠下面的分流规则判断发到哪个话题。
